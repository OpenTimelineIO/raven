...
bool Splitter(const char *str_id, bool split_vertically, float thickness,
              ...
              float splitter_long_axis_size, float hover_extend) {
  ...
  return ImGui::SplitterBehavior(
      ...
      min_size1, min_size2, hover_extend,
      0.0f // hover_visibility_delay
  );
}

bool KnobFloat(const char *label, float *v, float v_speed, float v_min,
               float v_max, const char *format, ImGuiSliderFlags flags) {
  return KnobScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max,
                    format, flags);
}

bool KnobInt(const char *label, int *v, float v_speed, int v_min, int v_max,
             const char *format, ImGuiSliderFlags flags) {
  return KnobScalar(label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max,
                    format, flags);
}
...
bool KnobScalar(const char *label, ImGuiDataType data_type, void *p_data,
  ...
  bool temp_input_is_active =
      temp_input_allowed && ImGui::TempInputIsActive(id);
  if (!temp_input_is_active) {
    const bool focus_requested =
        temp_input_allowed && ImGui::FocusableItemRegister(window, id);
  ...
  dl->PathLineTo(center);
  dl->PathStroke(ImGui::GetColorU32(ImGuiCol_Border), 0,
                 style.FrameBorderSize * 2);

  dl->AddCircleFilled(center, w / 4,
                      IM_COL32_BLACK); // ImGui::GetColorU32(ImGuiCol_FrameBg));

  // Drag behavior
  const bool value_changed = ImGui::DragBehavior(id, data_type, p_data, v_speed,
