...
void DeleteSelectedObject() {
  ...
  if (const auto &selected_composable =
          dynamic_cast<otio::Composable *>(appState.selected_object)) {
    if (const auto &parent = selected_composable->parent()) {
      auto &children = parent->children();
      auto it =
          std::find(children.begin(), children.end(), selected_composable);
      if (it != children.end()) {
  ...
  if (const auto &selected_marker =
          dynamic_cast<otio::Marker *>(appState.selected_object)) {
    if (const auto &item =
            dynamic_cast<otio::Item *>(appState.selected_context)) {
  ...
  if (const auto &selected_effect =
          dynamic_cast<otio::Effect *>(appState.selected_object)) {
    if (const auto &item =
            dynamic_cast<otio::Item *>(appState.selected_context)) {
...
void AddMarkerAtPlayhead(otio::Item *item, std::string name,
                         std::string color) {
  ...
  if (item == NULL) {
    if (const auto &selected_item =
            dynamic_cast<otio::Item *>(appState.selected_object)) {
      item = selected_item;
    } else {
  ...
  auto time =
      timeline->tracks()->transformed_time(playhead, item, &error_status);
  if (otio::is_error(error_status)) {
    Message("Error transforming time: %s",
  ...
  otio::SerializableObject::Retainer<otio::Marker> marker =
      new otio::Marker(name, marked_range, color);
...
void AddTrack(std::string kind) {
  ...
  if (stack) {
    otio::SerializableObject::Retainer<otio::Track> new_track =
        new otio::Track("", nonstd::nullopt, kind);
    ...
    if (otio::is_error(error_status)) {
      Message("Error inserting track: %s",
              otio_error_string(error_status).c_str());
      return;
    }
  }
...
void FlattenTrackDown() {
  ...
  auto track_below =
      dynamic_cast<otio::Track *>(children[selected_index - 1].value);
  if (track_below == NULL) {
