...
void DrawItem(otio::Item *item, float scale, ImVec2 origin, float height,
  ...
  ImVec2 render_pos(item_range.start_time().to_seconds() * scale + origin.x,
                    ImGui::GetCursorPosY());
  ...
  if (fancy_corners) {
    ...
    // top edge
    draw_list->AddLine(ImVec2(p0.x + corner_radius, p0.y), ImVec2(p1.x, p0.y),
                       light_edge_color);
    // bottom edge
    draw_list->AddLine(ImVec2(p0.x, p1.y - 1),
                       ImVec2(p1.x - corner_radius, p1.y - 1), dark_edge_color);
  } else {
...
void DrawTransition(otio::Transition *transition, float scale, ImVec2 origin,
                    float height,
                    std::map<otio::Composable *, otio::TimeRange> &range_map) {
...
void DrawEffects(otio::Item *item, float scale, ImVec2 origin, float row_height,
                 std::map<otio::Composable *, otio::TimeRange> &range_map) {
  ...
  if (size.x > text_size.x && label_str != "") {
    const ImVec2 text_pos = ImVec2(p0.x + size.x / 2 - text_size.x / 2,
...
void DrawMarkers(otio::Item *item, float scale, ImVec2 origin, float height,
                 std::map<otio::Composable *, otio::TimeRange> &range_map) {
  ...
  for (const auto &marker : markers) {
    ...
    draw_list->AddTriangleFilled(
        ImVec2(p0.x, p0.y), ImVec2(p0.x + arrow_width / 2, p1.y),
        ImVec2(p0.x + arrow_width / 2, p0.y), fill_color);
    draw_list->AddRectFilled(ImVec2(p0.x + arrow_width / 2, p0.y),
                             ImVec2(p1.x - arrow_width / 2, p1.y),
                             ImColor(dimmed_fill_color));
    draw_list->AddTriangleFilled(ImVec2(p1.x - arrow_width / 2, p0.y),
                                 ImVec2(p1.x - arrow_width / 2, p1.y),
                                 ImVec2(p1.x, p0.y), fill_color);

    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "%s: %s\nColor: %s\nRange: %s - %s\nDuration: %s",
          marker->schema_name().c_str(), marker->name().c_str(),
          marker->color().c_str(),
...
void DrawObjectLabel(otio::SerializableObjectWithMetadata *object,
                     float height) {
  ...
  char label_str[200];
  snprintf(label_str, sizeof(label_str), "%s: %s",
           object->schema_name().c_str(), object->name().c_str());
...
void DrawTrackLabel(otio::Track *track, int index, float height) {
  ...
  char label_str[200];
  snprintf(label_str, sizeof(label_str), "%c%d: %s", track->kind().c_str()[0],
           index, track->name().c_str());
  ...
  if (ImGui::IsItemHovered()) {
    ...
    ImGui::SetTooltip(
        "%s: %s\n%s #%d\nRange: %s - %s\nDuration: %s\nChildren: %ld",
        track->schema_name().c_str(), track->name().c_str(),
        track->kind().c_str(), index,
        FormattedStringFromTime(trimmed_range.start_time()).c_str(),
        FormattedStringFromTime(trimmed_range.end_time_inclusive()).c_str(),
...
void DrawTrack(otio::Track *track, int index, float scale, ImVec2 origin,
  ...
  if (otio::is_error(error_status)) {
    Message("Error calculating timing: %s",
...
void DrawTimecodeRuler(const void *ptr_id, otio::RationalTime start,
  ...
  auto zebra_color_light =
      ImColor(255, 255, 255, 255.0 * appState.zebra_factor);
  ...
  for (int tick_index = 0; tick_index < tick_count; tick_index++) {
    ...
    if (seconds_per_tick >= 0.5) {
      ...
      draw_list->AddRectFilled(zebra_start, zebra_end,
                               (frame & 1) ? zebra_color_dark
                                           : zebra_color_light);
    }

    const ImVec2 tick_label_pos =
...
bool DrawTimecodeTrack(otio::RationalTime start, otio::RationalTime end,
                       float frame_rate, float scale, float full_width,
                       float track_height, bool interactive = true) {
  ...
  ImGui::SetCursorPos(old_pos);
  DrawTimecodeRuler("##TimecodeTrackRuler", start, end, frame_rate, 1.0, scale,
                    size.x, size.y);
...
ImColor ColorOverColor(ImColor a, ImColor b) {
  ...
  return ImColor(a.Value.x * alpha + b.Value.x * (1.0 - alpha),
                 a.Value.y * alpha + b.Value.y * (1.0 - alpha),
                 a.Value.z * alpha + b.Value.z * (1.0 - alpha), b.Value.w);
}

float DrawPlayhead(otio::RationalTime start, otio::RationalTime end,
                   otio::RationalTime playhead, float scale, float full_width,
                   float track_height, float full_height, ImVec2 origin,
                   bool draw_arrow) {
  ...
  auto playhead_label_bg_color =
      ColorOverColor(playhead_fill_color, background_color);
...
bool DrawTransportControls(otio::Timeline *timeline) {
  ...
  if (ImGui::SliderFloat("##Zoom", &appState.scale, 0.1f, 5000.0f, "Zoom",
  ...
  const float columnWidth =
      ImGui::GetColumnWidth(1) - GImGui->Style.ScrollbarSize;
  const ImU32 pz_inactive_color =
      ImGui::ColorConvertFloat4ToU32(GImGui->Style.Colors[ImGuiCol_Button]);
  ...
  ImVec2 tl_end(startx + columnWidth,
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    ImGui::PushID(i);
    ImGui::InvisibleButton("zoompanner",
  ...
  win->DrawList->AddRectFilled(tl_start, tl_end,
...
void DrawTimeline(otio::Timeline *timeline) {
  ...
  if (ImGui::BeginTable("Tracks", 2, flags)) {
    ...
    if (ImGui::GetFrameCount() >
        1) { // crash if we call this on the 1st frame?!
    ...
    if (DrawTimecodeTrack(start, end, playhead.rate(), appState.scale,
                          full_width, appState.track_height)) {
      // scroll_to_playhead = true;
    }

    std::map<otio::Composable *, otio::TimeRange> empty_map;
    DrawMarkers(timeline->tracks(), appState.scale, origin,
                appState.track_height, empty_map);

    // draw just the top of the playhead in the fixed timecode track
    float playhead_x = DrawPlayhead(start, end, playhead, appState.scale,
                                    full_width, appState.track_height,
                                    appState.track_height, origin, true);
    ...
    // for (const auto& video_track : video_tracks)
    {
      ...
      if (ImGui::TableNextColumn()) {
        DrawTrack(video_track, index, appState.scale, origin, full_width,
    ...
    for (const auto &audio_track : audio_tracks) {
      ...
      if (ImGui::TableNextColumn()) {
        DrawTrack(audio_track, index, appState.scale, origin, full_width,
                  appState.track_height);
      }
      index++;
    }
    ...
    ImGui::TableNextColumn();
    playhead_x =
        DrawPlayhead(start, end, playhead, appState.scale, full_width,
                     appState.track_height, full_height, origin, false);
